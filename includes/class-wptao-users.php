...
class WTBP_WPTAO_Users extends WTBP_WPTAO_DB {
	...
	public function display_name( $mixed ) {
		...
		if ( empty( $name ) ) {
			...
			if ( isset( $object->email ) && is_email( $object->email ) ) {
				...
				$name = sprintf( __( 'Unknown name (%d)', WTBP_WPTAO_DOMAIN ), $user_id );
			}
		}
	...
	public function parse_status( $status_in, $html = false ) {
		...
		switch ( $status_in ) {
			case 'valid':
				$status_out = __( 'Valid', WTBP_WPTAO_DOMAIN );
				break;

			case 'disposable':
				$status_out = __( 'Disposable', WTBP_WPTAO_DOMAIN );
				break;

			case 'invalid':
				$status_out = __( 'Invalid', WTBP_WPTAO_DOMAIN );
				break;

			case 'blacklist':
				$status_out = __( 'Blacklisted', WTBP_WPTAO_DOMAIN );
				break;

			default:
				$status_out = __( 'Undeterminied', WTBP_WPTAO_DOMAIN );
		}
