...
function wtbp_wptao_show_upgrade_notices() {
	...
	if ( version_compare( $wptao_version, '1.0.3', '<' ) || !WTBP_WPTAO_Helpers::has_upgrade_completed( 'remove_port_from_pageviews' ) ) {
		...
		if ( WTBP_WPTAO_Helpers::has_ports_in_pageviews() ) {
			printf(
			'<div class="updated"><p>' . esc_html__( 'WP Tao needs to upgrade the pageview events, click %shere%s to start the upgrade.', WTBP_WPTAO_DOMAIN ) . '</p></div>', '<a href="' . esc_url( admin_url( 'index.php?page=wtbp-wptao-upgrades&wptao-upgrade=remove_port_from_pageviews' ) ) . '">', '</a>'
			);
		} else {
	...
	if ( version_compare( $wptao_version, '1.1.1', '<' ) || !WTBP_WPTAO_Helpers::has_upgrade_completed( 'update_edd_currency_for_order_payment' ) ) {
		...
		if ( WTBP_WPTAO_Helpers::has_no_currency_in_edd_order_payment() ) {
			printf(
			'<div class="updated"><p>' . esc_html__( 'WP Tao needs to upgrade the order and payment events, click %shere%s to start the upgrade.', WTBP_WPTAO_DOMAIN ) . '</p></div>', '<a href="' . esc_url( admin_url( 'index.php?page=wtbp-wptao-upgrades&wptao-upgrade=update_edd_currency_for_order_payment' ) ) . '">', '</a>'
			);
		} else {
	...
	if ( version_compare( $wptao_version, '1.1.3', '<' ) || !WTBP_WPTAO_Helpers::has_upgrade_completed( 'update_status_for_identified' ) ) {
		...
		if ( WTBP_WPTAO_Helpers::check_statuses_determined() ) {
			printf(
			'<div class="updated"><p>' . esc_html__( 'WP Tao needs to upgrade the data of those identified, click %shere%s to start the upgrade.', WTBP_WPTAO_DOMAIN ) . '</p></div>', '<a href="' . esc_url( admin_url( 'index.php?page=wtbp-wptao-upgrades&wptao-upgrade=update_status_for_identified' ) ) . '">', '</a>'
			);
		} else {
	...
	if ( version_compare( $wptao_version, '1.1.9.4', '<' ) || !WTBP_WPTAO_Helpers::has_upgrade_completed( 'update_events_missing_user_id' ) ) {
		...
		if ( WTBP_WPTAO_Helpers::check_events_missing_user_id() ) {
			printf(
			'<div class="updated"><p>' . esc_html__( 'WP Tao needs to upgrade the events data, click %shere%s to start the upgrade.', WTBP_WPTAO_DOMAIN ) . '</p></div>', '<a href="' . esc_url( admin_url( 'index.php?page=wtbp-wptao-upgrades&wptao-upgrade=update_events_missing_user_id' ) ) . '">', '</a>'
			);
		} else {
	...
	if ( version_compare( $wptao_version, '1.2.2', '<' ) || !WTBP_WPTAO_Helpers::has_upgrade_completed( 'v122_update_users_meta' ) ) {
		...
		printf(
		'<div class="updated"><p>' . esc_html__( 'WP Tao needs to upgrade the users data, click %shere%s to start the upgrade.', WTBP_WPTAO_DOMAIN ) . '</p></div>', '<a href="' . esc_url( admin_url( 'index.php?page=wtbp-wptao-upgrades&wptao-upgrade=v122_update_users_meta' ) ) . '">', '</a>'
		);
	}
...
function wtbp_wptao_v103_remove_port_from_pageviews() {
	...
	if ( !current_user_can( 'manage_options' ) ) {
		wp_die( __( 'You do not have permission to perform WP Tao upgrades', WTBP_WPTAO_DOMAIN ), __( 'Error', WTBP_WPTAO_DOMAIN ), array( 'response' => 403 ) );
	}
