...
function wtbp_wptao_hint_abandoned_carts_vs_add() {
	...
	if ( $total_abandoned > 10 && $total_completed > 10 ) {
		...
		if ( $abandoned_percent > $percent_tresh ) {
			...
			$title = sprintf( __( 'High abandoned carts rate! (%.2f%%)', WTBP_WPTAO_DOMAIN ), $abandoned_percent );
			...
			$content = sprintf( __( 'It means that about %.2f%% of carts were abandoned in the last 30 days.', WTBP_WPTAO_DOMAIN ), $abandoned_percent ) . '<br /><br />';

			$content .= __( 'Do you know that you can increase your sales right now? Just start recovering the abandoned carts automatically!', WTBP_WPTAO_DOMAIN ) . '<br />';
			$content .= sprintf( __( 'For more information please check out the %sWP Tao Recover Lost Sales addon (click)%s!', WTBP_WPTAO_DOMAIN ), '<a href="' . $url . '" target="_blank">', '</a>' ) . '<br />';
		}
	}
...
function wtbp_wptao_hint_recently_identified() {
	...
	$title = __( 'Warning about the collected contacts!', WTBP_WPTAO_DOMAIN );

	$content = sprintf( __( 'During the past 30 days you collected <b>%d</b> contacts in total.', WTBP_WPTAO_DOMAIN ), ($total_valid + $total_wrong ) ) . ' ';
	$content .= sprintf( __( '<b>%d</b> of them were valid, <b>%d</b> invalid, <b>%d</b> disposable and <b>%d</b> blacklisted.', WTBP_WPTAO_DOMAIN ), $total_valid, $total_invalid, $total_disposable, $total_blacklist ) . '<br />';
...
function wtbp_wptao_hint_mailchimp_identified() {
	...
	$title = __( 'You can identify MailChimp contacts as soon as they enter the site!', WTBP_WPTAO_DOMAIN );

	$target = 'addons/wp-tao-mailchimp/?utm_source=plugin&utm_medium=hint&utm_campaign=wptao_hint_mc';

	$url = WTBP_WPTAO_Helpers::get_wptao_url( $target );

	$content = __( 'WP Tao has detected that you are a MailChimp user. Do you know that you can identify MailChimp contacts as soon as they enter the site?', WTBP_WPTAO_DOMAIN ) . '<br />';
	$content .= sprintf( __( 'For more information please check out the %sWP Tao MailChimp integration (click)%s!', WTBP_WPTAO_DOMAIN ), '<a href="' . $url . '" target="_blank">', '</a>' ) . '<br />';
...
function wtbp_wptao_hint_sales_campaigns_mailchimp() {
	...
	$title = __( 'You can track the results of MailChimp campaigns here!', WTBP_WPTAO_DOMAIN );

	$target = 'addons/wp-tao-mailchimp/?utm_source=plugin&utm_medium=hint&utm_campaign=wptao_hint_mc';

	$url = WTBP_WPTAO_Helpers::get_wptao_url( $target );

	$content = __( 'WP Tao has detected that you are a MailChimp user. Do you know that you can track the results of MailChimp campaigns here?', WTBP_WPTAO_DOMAIN ) . '<br />';
	$content .= sprintf( __( 'For more information please check out the %sWP Tao MailChimp integration (click)%s!', WTBP_WPTAO_DOMAIN ), '<a href="' . $url . '" target="_blank">', '</a>' ) . '<br />';
