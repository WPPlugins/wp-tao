...
class WTBP_WPTAO_Admin_Report_Basic_User_Identified_Contacts extends WTBP_WPTAO_Admin_Reports {
	...
	function __construct() {
		...
		$this->report_name = __( 'Recently identified', WTBP_WPTAO_DOMAIN );
	...
	private function contacts_widget() {
		...
		// defaults
		$html = array(
			'valid'		 => sprintf( __( '<li>%s: 0</li>', WTBP_WPTAO_DOMAIN ), TAO()->users->parse_status( 'valid' ) ),
			'invalid'	 => sprintf( __( '<li>%s: 0</li>', WTBP_WPTAO_DOMAIN ), TAO()->users->parse_status( 'invalid' ) ),
			'disposable' => sprintf( __( '<li>%s: 0</li>', WTBP_WPTAO_DOMAIN ), TAO()->users->parse_status( 'disposable' ) ),
			'blacklist'	 => sprintf( __( '<li>%s: 0</li>', WTBP_WPTAO_DOMAIN ), TAO()->users->parse_status( 'blacklist' ) ),
		);
		...
		if ( !empty( $value ) && is_array( $value ) ) {
			foreach ( $value as $item ) {
				$html[ $item->status ] = sprintf( __( '<li>%s: %d</li>', WTBP_WPTAO_DOMAIN ), TAO()->users->parse_status( $item->status ), $item->cnt );
				$cnt += $item->cnt;
		...
		if ( $cnt == 0 ) {
			$value_text = __( 'No results!', WTBP_WPTAO_DOMAIN );
		} else if ( $refs ) {
			$lp		 = 0;
			$cnt_ref = 0;
			$value_text .= '<ol class="wptao-dbox-list">';
			...
			foreach ( $refs as $ref ) {
				$value_text .= sprintf( __( '<li>%s (%d)</li>', WTBP_WPTAO_DOMAIN ), TAO()->traffic->get_source_analyzed( null, $ref->referer, array( 'noprot' => true ) ), $ref->cnt );
				$cnt_ref += $ref->cnt;
				$lp++;
				if ( $lp == 3 )
					break;
			...
			$cnt_dif = $cnt - $cnt_ref;
			if ( $cnt_dif > 0 ) {
				$value_text .= sprintf( __( '<li>Other in total (%d)</li>', WTBP_WPTAO_DOMAIN ), $cnt_dif );
			}
		...
		$args = array(
			...
			'category'		 => 'user',
			'priority'		 => 50,
			'title'			 => __( 'Recently identified', WTBP_WPTAO_DOMAIN ),
