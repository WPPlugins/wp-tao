...
class WTBP_WPTAO_Admin_Report_Basic_Sale_User extends WTBP_WPTAO_Admin_Reports {
	...
	function __construct() {
		...
		$this->report_name = __( 'Best buyers', WTBP_WPTAO_DOMAIN );
	...
	private function sales_widget() {
		...
		if ( !empty( $this->user_id ) ) {
			...
			$widget_content	 = sprintf( _n( '%s<br />Total: %s (%d sale)', '%s<br />Total: %s (%d sales)', $this->total_orders, WTBP_WPTAO_DOMAIN ), $user_txt, WTBP_WPTAO_Helpers::amount_format( $this->total_amount, $wptao_settings[ 'currency' ] ), $this->total_orders );
		} else {
			$widget_content = __( 'No results!', WTBP_WPTAO_DOMAIN );
		}
		...
		$args = array(
			...
			'size'			 => 'middle',
			'category'		 => 'commerce',
			'title'			 => __( 'Best buyer', WTBP_WPTAO_DOMAIN ),
	...
	private function churn_widget() {
		...
		$args = array(
			...
			'size'			 => 'small',
			'category'		 => 'commerce',
			'title'			 => __( 'Churn rate', WTBP_WPTAO_DOMAIN ),
			'report_slug'	 => $this->report_slug, // Add link to a report
			'value_text'	 => $widget_content,
			'dashicon'		 => 'dashicons-cart',
			'priority'		 => 51
	...
	private function prepare_churn( $users ) {
		...
		$churn = array(
			'new'		 => __( 'No data!', WTBP_WPTAO_DOMAIN ),
			'returning'	 => __( 'No data!', WTBP_WPTAO_DOMAIN ),
			'churn_rate' => __( 'No data!', WTBP_WPTAO_DOMAIN )
		);
