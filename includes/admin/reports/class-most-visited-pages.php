...
class WTBP_WPTAO_Admin_Report_Most_Visited_Pages extends WTBP_WPTAO_Admin_Reports {
	...
	function __construct() {
		...
		$this->report_name = __( 'Most visited pages', WTBP_WPTAO_DOMAIN );
	...
	private function pages_widget() {
		...
		if ( !empty( $value ) && is_array( $value ) ) {
			...
			$value_text .= '<li>' . __( 'No pages found', WTBP_WPTAO_DOMAIN ) . '</li>';
		}
		...
		$args = array(
			...
			'category'		 => 'traffic',
			'priority'		 => 48,
			'title'			 => __( 'Most visited pages', WTBP_WPTAO_DOMAIN ),
	...
	public function page_link_format( $object, $in_widget = false ) {
		...
		$anchor = __( 'Unrecognized page', WTBP_WPTAO_DOMAIN );
		...
		if ( !empty( $post_id ) || is_numeric( $url ) ) {
			...
			if ( absint( get_option( 'page_on_front' ) ) === $post_id ) {
				$anchor = sprintf( __( '%s (Front Page)', WTBP_WPTAO_DOMAIN ), $anchor );
			}
		} else {
		...
		// if is homepage
		if ( home_url() === $url ) {
			$anchor = __( 'Home page', WTBP_WPTAO_DOMAIN );
		}
