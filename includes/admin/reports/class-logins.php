...
class WTBP_WPTAO_Admin_Report_Logins extends WTBP_WPTAO_Admin_Reports {
	...
	function __construct() {
		...
		$this->report_name = __( 'User logins', WTBP_WPTAO_DOMAIN );
	...
	private function login_widget() {
		...
		$value .= '<h5>' . __( 'Success', WTBP_WPTAO_DOMAIN ) . '</h5>';
		$value .= '<span>' . absint( $this->total_success_logins ) . '</span>';
		$value .= '</div>';
		$value .= '<div class="wptao-dbox-slogin">';
		$value .= '<h5>' . __( 'Failed', WTBP_WPTAO_DOMAIN ) . '</h5>';
		...
		$args = array(
			...
			'category'		 => 'user',
			'priority'		 => 49,
			'title'			 => __( 'Logins', WTBP_WPTAO_DOMAIN ),
