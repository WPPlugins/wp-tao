...
class WTBP_WPTAO_Admin_Report_Frequency_Search extends WTBP_WPTAO_Admin_Reports {
	...
	function __construct() {
		...
		$this->report_name = __( 'Most frequently searched keywords', WTBP_WPTAO_DOMAIN );
	...
	private function search_widget() {
		...
		if ( !empty( $value[ 0 ] ) ) {
			...
			$value = __( 'No results!', WTBP_WPTAO_DOMAIN );
		}
		...
		$args = array(
			...
			'category'		 => 'traffic',
			'priority'		 => 47,
			'title'			 => __( 'Most frequently searched keyword', WTBP_WPTAO_DOMAIN ),
