...
class WTBP_WPTAO_Admin_Report_Basic_Sale_Total extends WTBP_WPTAO_Admin_Reports {
	...
	function __construct() {
		...
		$this->report_name = __( 'Total sales', WTBP_WPTAO_DOMAIN );
	...
	private function sales_widget() {
		...
		$widget_content = sprintf( _n( 'Total: %s (%d sale)<br />Average: %s', 'Total: %s (%d sales)<br />Average: %s', $this->total_orders, WTBP_WPTAO_DOMAIN ), WTBP_WPTAO_Helpers::amount_format( $this->total_amount, $wptao_settings[ 'currency' ] ), $this->total_orders, WTBP_WPTAO_Helpers::amount_format( $average, $wptao_settings[ 'currency' ] ) );

		$args = array(
			'id'			 => 'basic_sale_total',
			'size'			 => 'middle',
			'category'		 => 'commerce',
			'title'			 => __( 'Sales', WTBP_WPTAO_DOMAIN ),
	...
	public function print_script() {
		...
		<script>
		    ...
		    function wptao_chart_basic_sale_amount() {
		        ...
		        data.addColumn( 'date', '<?php _e( 'Date', WTBP_WPTAO_DOMAIN ); ?>' );
		        data.addColumn( 'number', '<?php _e( 'Amount', WTBP_WPTAO_DOMAIN ); ?>' );
		...
		?>
		    ...
		    function wptao_chart_basic_sale_orders() {
		        ...
		        var data = new google.visualization.DataTable();
		        data.addColumn( 'date', '<?php _e( 'Date', WTBP_WPTAO_DOMAIN ); ?>' );
		        data.addColumn( 'number', '<?php _e( 'Orders', WTBP_WPTAO_DOMAIN ); ?>' );
