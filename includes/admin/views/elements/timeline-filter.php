...
<div class="wptao-filter-wrap wptao-filter-events-page" id="wptao-timeline-filter">
	...
	<form id="wptao-timeline-form" method="POST" action="<?php echo esc_url( $this->base_url ); ?>">
		...
		<h2><?php _e( 'Timeline filter', WTBP_WPTAO_DOMAIN ); ?></h2>
		...
		<div class="wptao-module-content">
			...
			<div class="wptao-event-filter-dates">
				<h4><?php _e( 'Date', WTBP_WPTAO_DOMAIN ); ?></h4>
				<?php
			...
			<?php if ( isset( $filter[ 'categories' ] ) && is_array( $filter[ 'categories' ] ) && !empty( $filter[ 'categories' ] ) ): ?>
				<h4><?php _e( 'Categories', WTBP_WPTAO_DOMAIN ); ?></h4>
			...
			<?php if ( isset( $filter[ 'actions' ] ) && is_array( $filter[ 'actions' ] ) && !empty( $filter[ 'actions' ] ) ): ?>
				<h4><?php _e( 'Actions', WTBP_WPTAO_DOMAIN ); ?></h4>
				<div class="metabox-prefs wptao-event-filter-cat">
			...
			<?php if ( isset( $filter[ 'tags' ] ) && is_array( $filter[ 'tags' ] ) && !empty( $filter[ 'tags' ] ) ): ?>
				<h4><?php _e( 'Tags', WTBP_WPTAO_DOMAIN ); ?></h4>
			...
			<h4><?php _e( 'Display', WTBP_WPTAO_DOMAIN ); ?></h4>		

			<?php if ( TAO()->booleans->is_page_events === true ): ?>
				<div class="metabox-prefs wptao-event-filter-ident">
					<label for="wptao-show-by-ident"><?php _e( 'Identification: ', WTBP_WPTAO_DOMAIN ); ?>
						<select id="wptao-show-by-ident" name="wptao-show-by-ident" >
							<option value="both" <?php echo!isset( $_GET[ 'identified' ] ) || !in_array( $_GET[ 'identified' ], array( '0', '1' ) ) ? 'selected="selected"' : ''; ?>><?php _e( 'Show all', WTBP_WPTAO_DOMAIN ); ?></option>
							<option value="identified" <?php echo isset( $_GET[ 'identified' ] ) && $_GET[ 'identified' ] === '1' ? 'selected="selected"' : ''; ?>><?php _e( 'Identified only', WTBP_WPTAO_DOMAIN ); ?></option>
							<option value="unidentified" <?php echo isset( $_GET[ 'identified' ] ) && $_GET[ 'identified' ] === '0' ? 'selected="selected"' : ''; ?>><?php _e( 'Unidentified only', WTBP_WPTAO_DOMAIN ); ?></option>
						</select>
				</div>	
			<?php endif; ?>

			<label for="edit_post_per_page"><?php _e( 'Events per page:', WTBP_WPTAO_DOMAIN ); ?> <input type="number" name="wptao-events-number" class="small-text" value="<?php echo isset( $_GET[ 'ipp' ] ) && is_numeric( $_GET[ 'ipp' ] ) ? (int) abs( $_GET[ 'ipp' ] ) : esc_attr( TAO()->events->events_per_page ); ?>" /></label>
			...
			<div class="wptao-profile-sort-submit">
				...
				<button type="submit" class="button wptao-btn-sort button-primary"><?php _e( 'Sort', WTBP_WPTAO_DOMAIN ); ?></button>
