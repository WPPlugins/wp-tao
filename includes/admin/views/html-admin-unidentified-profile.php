...
<div class="wrap">
	...
	<div class="wptao-dashboard-head">
		<h2><span class="dashicons dashicons-id"></span><?php _e( 'Undifentified profile', WTBP_WPTAO_DOMAIN ); ?></h2>
	</div>
    ...
    <div id="wptao-user-profile" class="wptao-user-profile wptao-row">
		...
		<div class="wptao-back">
			<a href="<?php echo admin_url( 'admin.php?page=wtbp-wptao-events' ); ?>" >
				...
				<?php _e( 'Back to events timeline', WTBP_WPTAO_DOMAIN ); ?>
			</a>
		</div>
        ...
        <div class="wptao-pcol-1">
            ...
            <div class="wptao-module wptao-user-name wptao-module-header wptao-row">
                ...
                <h1><?php printf( __( 'Unidentified user (%d)', WTBP_WPTAO_DOMAIN ), $fp->fp_data->id ); ?></h1>
            </div>


				<div class="wptao-module wptao-mod-user-fingerprints">
					<h2><?php _e( 'Who is it?', WTBP_WPTAO_DOMAIN ) ?></h2>
					<?php
	
						echo '<p class="wptao-mod-user-fingerprint">';
						echo '<b>' . __( 'Fingerprint ID:', WTBP_WPTAO_DOMAIN ) . '</b> ' . esc_html($fp->fp_data->id) . '<br />';
						echo '<b>' . __( 'Created:', WTBP_WPTAO_DOMAIN ) . '</b> ' . WTBP_WPTAO_Helpers::get_date( "d.m.Y, H:i:s", $fp->fp_data->created_ts ) . '<br />';
						echo '<b>' . __( 'IP:', WTBP_WPTAO_DOMAIN ) . '</b> ' . esc_html($fp->fp_data->ip) . '<br />';
						echo '<b>' . __( 'User agent:', WTBP_WPTAO_DOMAIN ) . '</b> ' . esc_html($fp->fp_data->user_agent);
						echo '</p>';
