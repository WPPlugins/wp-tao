	...
	class WTBP_WPTAO_Admin_Dashboard {
		...
		public function register_widget( $args ) {
			...
			$defaults = array(
				...
				'report_link'	 => '',
				'report_text'	 => __( 'See a complete report', WTBP_WPTAO_DOMAIN ), // Add report link anchor.
				'priority'		 => 50,
				'value_text'	 => '',
				'value_number'	 => '',
				'dashicon'		 => '',
			...
			if ( !empty( $this->widgets ) ) {
				...
				foreach ( $widgets as $widget ) {
					...
					if ( !empty( $report_link ) ) {
						...
						if ( $total_hints > 0 ) {
							$html .= sprintf( '<a href="%s" title="%s" class="wptao-dbox-num-hints"><span class="dashicons dashicons-lightbulb"></span><span class="wpta-hints-num">%d</span></a>', esc_url( $report_link ), __( 'Show hints', WTBP_WPTAO_DOMAIN ), $total_hints );
						}
					...
					// Draggable handler
					$html .= '<span data-nonce="' . wp_create_nonce( 'hide-' . sanitize_title( $widget->id ) ) . '" title="' . __( 'Hide espresso report', WTBP_WPTAO_DOMAIN ) . '" class="wptao-dbox-close dashicons dashicons-no-alt"></span>';
