...
function wtbp_wptao_event_add_to_cart_title( $title, $event ) {
	...
	if ( isset( $event->meta[ 'edd_download_id' ] ) ) {
		...
		if ( !empty( $download_id ) ) {
			...
			$title = sprintf( __( '<a href="%s">%s</a> was added to the cart', WTBP_WPTAO_DOMAIN ), get_permalink( $download_id ), esc_attr( $event->value ) );
		}
	}
	...
	if ( isset( $event->meta[ 'woo_product_id' ] ) ) {
		...
		if ( !empty( $product_id ) ) {
			$title = sprintf( __( '<a href="%s">%s</a> was added to the cart', WTBP_WPTAO_DOMAIN ), get_permalink( $product_id ), esc_attr( $event->value ) );
		}
	}
