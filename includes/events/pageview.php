...
function wtbp_wptao_event_pageview_fire() {
	...
	$args[ 'title' ] = __( 'Pageview', WTBP_WPTAO_DOMAIN );
...
function wtbp_wptao_event_pageview_title( $title, $event ) {
	...
	if ( filter_var( $event->value, FILTER_VALIDATE_URL ) !== FALSE ) {
		$title = sprintf( __( '<a href="%s">Visited page</a>', WTBP_WPTAO_DOMAIN ), esc_url( $event->value ) );
	} else {
		$title = __( 'Visited page', WTBP_WPTAO_DOMAIN );
	}


	// Catch the home page.
	if ( $home_url === $event->value || WTBP_WPTAO_Helpers::standardize_url( $div_url[ 0 ] ) === $home_url ) {
		$title	 = sprintf( __( '<a href="%s">Home page</a> was visited', WTBP_WPTAO_DOMAIN ), esc_url( $event->value ) );
		$is_home = true;
	}
	...
	if ( isset( $event->meta[ 'post_id' ] ) && is_numeric( $event->meta[ 'post_id' ] ) ) {
		...
		if ( isset( $post ) && !empty( $post ) && !$is_home ) {
			...
			// Default
			$title = sprintf( __( '<a href="%s">%s</a> was visited', WTBP_WPTAO_DOMAIN ), esc_url( $event->value ), $post_title );


			// Catch the Post
			if ( 'post' === get_post_type( $post_id ) ) {

				$title = sprintf( __( 'Post <a href="%s">%s</a> was visited', WTBP_WPTAO_DOMAIN ), esc_url( $event->value ), $post_title );
			}

			// Catch the Page
			if ( 'page' === get_post_type( $post_id ) ) {

				$title = sprintf( __( 'Page <a href="%s">%s</a> was visited', WTBP_WPTAO_DOMAIN ), esc_url( $event->value ), $post_title );
			}
		}
	}
...
function wtbp_wptao_event_pageview_desc( $description, $event ) {
	...
	// Visited URL
	if ( filter_var( $event->value, FILTER_VALIDATE_URL ) !== FALSE ) {
		$description .= '<span class="wptao-meta-title">' . __( 'Visited URL:', WTBP_WPTAO_DOMAIN ) . '</span> ' . esc_html( $event->value ) . '<br />';
	...
	if ( false != $src ) {
		$description .= '<span class="wptao-meta-title">' . __( 'Traffic source:', WTBP_WPTAO_DOMAIN ) . '</span> ';
		$src = esc_html( $src );
	...
	if ( isset( $event->meta[ 'query_string' ] ) ) {
		$description .= '<span class="wptao-meta-title">' . __( 'Query args:', WTBP_WPTAO_DOMAIN ) . '</span> ';
