...
function wtbp_wptao_event_contact_title( $title, $event ) {
	...
	if ( !empty( $event->value ) ) {
		...
		$title = sprintf( __( 'Contact from %s', WTBP_WPTAO_DOMAIN ), esc_attr( $event->value ) );
	}
...
function wtbp_wptao_event_contact_desc( $description, $event ) {
	...
	if ( !empty( $source ) ) {
		...
		$description .= '<span class="wptao-meta-title">' . __( 'Source:', WTBP_WPTAO_DOMAIN ) . '</span> ' . $source . '<br />';
	}
