...
function wtbp_wptao_event_login_successful_fire( $user_login, $user = false ) {
	...
	$args = array(
		'title'		 => __( 'Successful login', WTBP_WPTAO_DOMAIN ),
		'value'		 => 'ok',
		'tags'		 => array( 'wp' ),
...
function wtbp_wptao_event_login_failed_fire( $username ) {
	...
	$args = array(
		'title'	 => sprintf( __( 'Failed login. Tried to sign in as %s', WTBP_WPTAO_DOMAIN ), esc_attr( $username ) ),
		'value'	 => 'failed',
		'tags'	 => array( 'wp' ),
...
function wtbp_wptao_event_login_failed_title( $title, $event ) {
	...
	if ( !empty( $event->meta[ 'login' ] ) && ('failed' == $event->value) ) {
		...
		$title = sprintf( __( 'Failed login. Tried to sign in as <q>%s</q>', WTBP_WPTAO_DOMAIN ), esc_attr( $event->meta[ 'login' ] ) );
	} else if ( !empty( $event->meta[ 'wp_user_id' ] ) && ('ok' == $event->value) ) {

		$user_data = get_userdata( $event->meta[ 'wp_user_id' ] );
		if ( !empty( $user_data ) ) {
			$title = sprintf( __( 'Successful login as <a href="%s">%s</a>', WTBP_WPTAO_DOMAIN ), esc_url( get_edit_user_link( $event->meta[ 'wp_user_id' ] ) ), $user_data->user_login );
		}
	}
