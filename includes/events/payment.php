...
function wtbp_wptao_event_payment_woo_fire( $order_id ) {
	...
	$args[ 'title' ] = __( 'Payment', WTBP_WPTAO_DOMAIN );
...
function wtbp_wptao_event_payment_title( $title, $event ) {
	...
	$title = sprintf( __( 'New payment in the amount of %s', WTBP_WPTAO_DOMAIN ), esc_attr( $amount ) );
...
function wtbp_wptao_event_payment_desc( $description, $event ) {
	...
	if ( isset( $event->meta[ 'edd_payment_id' ] ) ) {
		...
		$description .= '<span class="wptao-meta-title">' . __( 'Link:', WTBP_WPTAO_DOMAIN ) . '</span> ';
		$description .= sprintf( '<a href="%s">' . __( 'Order #%d', WTBP_WPTAO_DOMAIN ) . '</a>', $url, $payment_id ) . '<br />';
	}
	...
	if ( isset( $event->meta[ 'edd_parent_id' ] ) ) {
		...
		$description .= '<span class="wptao-meta-title">' . __( 'Parent:', WTBP_WPTAO_DOMAIN ) . '</span> ';
		$description .= sprintf( '<a href="%s">' . __( 'Order #%d', WTBP_WPTAO_DOMAIN ) . '</a>', $url, $payment_id ) . '<br />';
	}
	...
	if ( isset( $event->meta[ 'woo_order_id' ] ) ) {
		...
		$description .= '<span class="wptao-meta-title">' . __( 'Link:', WTBP_WPTAO_DOMAIN ) . '</span> ';
		$description .= sprintf( '<a href="%s">' . __( 'Order #%d', WTBP_WPTAO_DOMAIN ) . '</a>', $url, $order_id ) . '<br />';
	}

	// sell platform
	if ( !empty( $sales_platform ) ) {
		$description .= '<span class="wptao-meta-title">' . __( 'Sales platform:', WTBP_WPTAO_DOMAIN ) . '</span> ' . $sales_platform;
	}
