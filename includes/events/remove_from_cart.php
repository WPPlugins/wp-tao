...
function wtbp_wptao_event_remove_from_cart_woo_fire( $cart_item_key, $wc_cart ) {
	...
	$args[ 'title' ] = __( 'Add to cart', WTBP_WPTAO_DOMAIN );
...
function wtbp_wptao_event_remove_from_cart_edd_fire( $cart_key ) {
	...
	$args[ 'title' ] = __( 'Remove from cart', WTBP_WPTAO_DOMAIN );
...
function wtbp_wptao_event_remove_from_cart_title( $title, $event ) {
	...
	if ( isset( $event->meta[ 'edd_download_id' ] ) ) {
		...
		if ( !empty( $download_id ) ) {
			...
			$title = sprintf( __( '<a href="%s">%s</a> was removed from cart', WTBP_WPTAO_DOMAIN ), get_permalink( $download_id ), esc_attr( $event->value ) );
		}
	}
	...
	if ( isset( $event->meta[ 'woo_product_id' ] ) ) {
		...
		if ( !empty( $product_id ) ) {
			$title = sprintf( __( '<a href="%s">%s</a> was removed from cart', WTBP_WPTAO_DOMAIN ), get_permalink( $product_id ), esc_attr( $event->value ) );
		}
	}
...
function wtbp_wptao_event_remove_from_cart_desc( $description, $event ) {
	...
	if ( isset( $event->meta[ 'edd_download_id' ] ) ) {
		...
		if ( TAO()->diagnostic->is_edd_enabled() === false ) {
			return __( 'Easy Digital Downloads is disabled. Activate the plugin to see more information.', WTBP_WPTAO_DOMAIN );
		}
		...
		//$description .= sprintf( __( 'Item price: %.2f.', WTBP_WPTAO_DOMAIN ), $amount );
		//$description .= ' ' . sprintf( __( 'Currency: %s.', WTBP_WPTAO_DOMAIN ), edd_get_currency() );
		$description .= '<span class="wptao-meta-title">' . __( 'Quantity:', WTBP_WPTAO_DOMAIN ) . '</span> ' . $event->meta[ 'edd_quantity' ] . '<br />';
	}

	// additional info (woo)
	if ( isset( $event->meta[ 'woo_product_id' ] ) ) {
		$description .= '<span class="wptao-meta-title">' . __( 'Quantity:', WTBP_WPTAO_DOMAIN ) . '</span> ' . $event->meta[ 'woo_quantity' ] . '<br />';
	}

	// sales platform
	if ( !empty( $sales_platform ) ) {
		$description .= '<span class="wptao-meta-title">' . __( 'Sales platform:', WTBP_WPTAO_DOMAIN ) . '</span> ' . $sales_platform . '<br />';
	}
